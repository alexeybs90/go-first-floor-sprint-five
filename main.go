...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(w.meanSpeed()*KmHInMsec, 2)/h)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinsInHour)
}
...
func (s Swimming) meanSpeed() float64 {
	...
	return float64(s.LengthPool) * float64(s.CountPool) / float64(MInKm) / h
}
